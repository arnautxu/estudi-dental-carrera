...
function card(guide, heading = 'h2') {
  return `<article class="blog-card" data-blog-category="${guide.category}">
    <a class="blog-card__link" href="/${guide.path}">
      <div class="blog-card__image"><img src="/${guide.image}" alt="${escape(guide.imageAlt)}" width="${guide.imageWidth}" height="${guide.imageHeight}"${guide.photography?.srcset ? ` srcset="${escape(guide.photography.srcset)}" sizes="(max-width: 640px) calc(100vw - 40px), (max-width: 1000px) 45vw, 400px"` : ''} loading="lazy" decoding="async" /></div>
